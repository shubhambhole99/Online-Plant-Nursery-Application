...
//	Planter
	...
	public Cart AddPlanterToCart(Integer customer_id,Integer planterId) {
		...
		if(cu!=null) {
			...
			Cart cn=cu.getOd();
			cn.getPlanterlist().add(splanter);
			cr.save(cn);
			cd.save(cu);
...
//				Plant p=temp.get(i);
//				p.setPlantstock(p.getPlantstock()-1);
//				pd.save(p);
//				if(p.getPlantstock()==0) {
//					break;
//				}
				neword.getPlantsbought().add(temp.get(i));
			}
			for(int i=0;i<temp1.size();i++) {
				Planter p=temp1.get(i);
...
//			
			...
			ord.save(neword);
			cu.getOl().add(neword);
			co.getPlanterlist().clear();
			co.getPlantlist().clear();
			co.getSeedlist().clear();
			co.setTotalitems(0);
			co.setTotalprice(0);
			cu.setOd(co);
			cr.save(co);
